...
#[guishaoli start]Make adaption to redmine3.4.6,replace 'issues' with 'queries',
#because fucntion 'query_links' is moved from issues_helper to queries_helper
#[guishaoli end]
	...
	def self.included(base)
		base.class_eval do
			...
			def charge_wrappers #Select every public wrappers, or privates wrapper of the current User
                                #[guishaoli start]use id to sort
				@wrappers = QueryWrapper.all.order("#{QueryWrapper.table_name}.id ASC").where(@project.nil? ? ["project_id IS NULL"] : ["project_id IS NULL OR project_id = ?", @project.id]).to_a
                                #[guishaoli end]
			...
			def query_links_with_plugin(title, queries, visibility) #Override with more content
			    ...
			    content_tag('h3', title) + "\n" +
                              #[guishaoli start]add class='queries_manage' to locate this 'ul'
			      content_tag('ul class="queries_manage"',
                              #[guishaoli end]
			      		@wrappers.collect {|w|
			      			content_tag('li',
                                                        #[guishaoli start]add style to satisfy UE's demand
			      				link_to(w.title, 'javascript:;', :style => "color: black;font-size: initial;",:onclick => "$('.content_wrap_id_'+"+w.id.to_s+").toggle();($(this).hasClass('collapsed') ? $(this).removeClass('collapsed') : $(this).addClass('collapsed'));", :class => 'collapsible collapsed wrap wrap_id_'+w.id.to_s,)+
                                                        #[guishaoli end]
			...
			def render_sidebar_queries_with_plugin(klass, project) #Override with a boolean parameter
			    ...
			    out = ''.html_safe
			    out << query_links(l(:label_my_queries), queries.select(&:is_private?),false)
			    out << query_links(l(:label_query_plural), queries.reject(&:is_private?),true)
			    out
			end
